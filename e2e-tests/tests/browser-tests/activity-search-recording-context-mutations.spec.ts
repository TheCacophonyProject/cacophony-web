...
import {signInExistingUser, waitToNavigateToProject, waitToNavigateToProjectPage} from "@/helpers/browse-helpers";

test("Can delete recordings in recording search context", async ({oneFrameCptv, page}) => {
    const {uploads, projectName} = await test.step("Init project with classified recordings, sign in user", async () => {
        const now = new Date();
    ...
    const recordingView = await test.step("Select middle recording, open recording modal", async () => {
