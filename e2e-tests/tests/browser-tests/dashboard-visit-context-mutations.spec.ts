...
test("Human tagging two different tracks with different classifications splits original visit in visit context list", async () => {
  // TODO
});
...
test("Deleting a single visit recording that was the only one in the list removes from the list and closes recording modal", async ({
  ...
  await test.step("Wait to navigate to the dashboard", async () => {
    await waitToNavigateToProject(page, projectName);
...
test("Dashboard with filtered visits should still advance to next/prev visits correctly upon visit split or deletion", async () => {
  // TODO
});
