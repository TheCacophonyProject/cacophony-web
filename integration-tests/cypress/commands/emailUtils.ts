...
export const waitForEmail = (type: string = "") => {
  ...
  return cy
    .exec(
      `cd ../api && docker exec cacophony-api bash -lic "until grep -q 'SERVER: received email' mailServerStub.log ; do sleep 0.1; done; cat mailServerStub.log;"`,
      { log: false },
    )
    .then((response) => {
...
export const startMailServerStub = () => {
  ...
  cy.exec(
    ...
    return cy.exec(
      `cd ../api && docker exec cacophony-api bash -lic "until [ -f mailServerStub.log ]; do sleep 0.1; done;"`,
      { log: false },
    );
  });
};
