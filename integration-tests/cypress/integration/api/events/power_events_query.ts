...
describe("Events - query power events", () => {
  ...
  const eventVersionData={"type": "versionData",
      "details": 
       {"modemd": "1.2.3",
        "audiobait": "3.0.1",
        "rtc-utils": "1.3.0",
  ...
  before(() => {
      ...
      cy.apiCreateUserGroupAndDevice("peOtherGroupAdmin", "peOtherGroup", "peOtherGroupCamera");

      //Create some events to reuse / query
      cy.apiEventsDeviceAddOnBehalf("peGroupAdmin", "peCamera", eventPowerOn, [time1]);
      cy.apiEventsDeviceAddOnBehalf("peGroupAdmin", "peCamera", eventStopReported, [time2]);
      cy.apiEventsDeviceAddOnBehalf("peGroupAdmin", "peCamera", eventPowerOff, [time3]);
      cy.apiEventsDeviceAddOnBehalf("peGroupAdmin", "peOtherCamera", eventPowerOn, [time4]);
      cy.apiEventsDeviceAddOnBehalf("peGroupAdmin", "peOtherCamera", eventStopReported, [time5]);
      cy.apiEventsDeviceAddOnBehalf("peOtherGroupAdmin", "peOtherGroupCamera", eventPowerOn, [time1]);
      cy.apiEventsDeviceAddOnBehalf("peOtherGroupAdmin", "peOtherGroupCamera", eventVersionData, [time4]);
      cy.apiEventsDeviceAddOnBehalf("peOtherGroupAdmin", "peOtherGroupCamera", eventStopReported, [time5]);
    });

  before(() => {
      peGroup={groupname: getTestName("peGroup"), id: getCreds("peGroup").id};
      peOtherGroup={groupname: getTestName("peOtherGroup"), id: getCreds("peOtherGroup").id};
      peCameraDevice={id: getCreds("peCamera").id, devicename: getTestName("peCamera"), GroupId: getCreds("peGroup").id, Group: peGroup};
      peOtherCameraDevice={id: getCreds("peOtherCamera").id, devicename: getTestName("peOtherCamera"), GroupId: getCreds("peGroup").id, Group: peGroup};
      peOtherGroupCameraDevice={id: getCreds("peOtherGroupCamera").id, devicename: getTestName("peOtherGroupCamera"), GroupId: getCreds("peOtherGroup").id, Group: peOtherGroup};

      expectedCamera={hasStopped: true, lastStarted: time1, lastReported: time2, lastStopped: time3, hasAlerted: true, Device: peCameraDevice};
      expectedOtherCamera={hasStopped: true, lastStarted: time4, lastReported: time5, lastStopped: null, hasAlerted: true, Device: peOtherCameraDevice};
      expectedOtherGroupCamera={hasStopped: true, lastStarted: time1, lastReported: time5, lastStopped: null, hasAlerted: true, Device: peOtherGroupCameraDevice};
  });

  it("Group admin can view all power events on all devices their group", () => {  
      cy.apiPowerEventsCheck("peGroupAdmin",undefined,{}, [expectedCamera, expectedOtherCamera]);
  });

  it("Group member can view all events on all devices their group", () => {  
      cy.apiPowerEventsCheck("peGroupMember",undefined,{}, [expectedCamera, expectedOtherCamera]);
  });
  ...
  it("Group admin can only request events from within their group", () => {
      ...
      cy.apiPowerEventsCheck("peGroupAdmin","peOtherCamera",{}, [expectedOtherCamera]);
