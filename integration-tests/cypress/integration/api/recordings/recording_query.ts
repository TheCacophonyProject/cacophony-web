...
let expectedRecording: ApiRecordingReturned[]=[];
...
describe("Recordings query using where", () => {
  before(() => {
    ...
    cy.apiRecordingAdd("rqCamera1b", recording3, undefined, "rqRecording3").then(() => {
        expectedRecording3 = TestCreateExpectedRecordingData( templateExpectedRecording, "rqRecording3", "rqCamera1b", "rqGroup", null, recording3);
        expectedRecording3.processingState='analyse';
    });
    cy.apiRecordingAdd("rqCamera1b", recording4, undefined, "rqRecording4").then(() => {
      expectedRecording4 = TestCreateExpectedRecordingData( templateExpectedRecording, "rqRecording4", "rqCamera1b", "rqGroup", null  , recording4);
      expectedRecording4.processingState='FINISHED';
      cy.testUserTagRecording(getCreds("rqRecording4").id, 0, "rqGroupAdmin", "possum");
      expectedRecording4.Tracks[0].TrackTags = [
        {
          ...
          data: null,
          TrackId: -99,
          User: {username: getTestName("rqGroupAdmin"), id: getCreds("rqGroupAdmin").id},
          UserId: getCreds("rqGroupAdmin").id,
        },
      ];
    });
    for ( let count=0; count < 20; count++ ) {
      let tempRecording=JSON.parse(JSON.stringify(recording1));
      //recordingDateTime order different to id order to test sort on different parameters
      tempRecording.recordingDateTime="2021-07-17T20:13:00."+(900-count).toString()+"Z"
      cy.apiRecordingAdd("rqCamera2", tempRecording, undefined, "rqRecordingB"+count.toString()).then(() => {
        expectedRecording[count] = TestCreateExpectedRecordingData( templateExpectedRecording, "rqRecordingB"+count.toString(), "rqCamera2", "rqGroup2", null, tempRecording);
      });
    }
  });
  ...
  it("Group member can query device's recordings", () => {
    cy.log("Check recording can be viewed correctly");
    cy.apiRecordingsQueryCheck( "rqGroupMember", {where: {id: getCreds("rqRecording1").id}}, [expectedRecording1],     EXCLUDE_IDS);
    cy.log("Check recording count can be viewed correctly");
    cy.apiRecordingsCountCheck( "rqGroupMember", {where: {id: getCreds("rqRecording1").id}}, 1);
  });
  ...
  it.skip("Device admin can query device's recordings", () => {
    cy.log("Check recording can be viewed correctly");
    cy.apiRecordingsQueryCheck( "rqDeviceAdmin", {where: {id: getCreds("rqRecording1").id}}, [expectedRecording1],     EXCLUDE_IDS);
    cy.log("Check recording count can be viewed correctly");
    cy.apiRecordingsCountCheck( "rqDeviceAdmin", {where: {id: getCreds("rqRecording1").id}}, 1);
  });
    
  it.skip("Device member can query device's recordings", () => {
    cy.log("Check recording can be viewed correctly");
    cy.apiRecordingsQueryCheck( "rqDeviceMember", {where: {id: getCreds("rqRecording1").id}}, [expectedRecording1],     EXCLUDE_IDS);
    cy.log("Check recording count can be viewed correctly");
    cy.apiRecordingsCountCheck( "rqDeviceMember", {where: {id: getCreds("rqRecording1").id}}, 1);
  });

  
  it("Non member cannot view devices recordings", () => {
    cy.log("Check no recordings returned");
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {id: getCreds("rqRecording1").id}}, [], EXCLUDE_IDS);
    cy.log("Check recording count can be viewed correctly");
    cy.apiRecordingsCountCheck( "rqGroup2Admin", {where: {id: getCreds("rqRecording1").id}}, 0);
  });
  ...
  it("Paging / sorting works as expected", () => {
    ...
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {}, offset:0, limit: 3, order: '[["id", "ASC"]]'}, expectedRecording.slice(0,3), EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroup2Admin", {where: {}, offset:0, limit: 3, order: '[["id", "ASC"]]'}, 20);

    cy.log("Get intermediate page, setting limit");
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {}, offset:3, limit: 3, order: '[["id", "ASC"]]'}, expectedRecording.slice(3,6), EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroup2Admin", {where: {}, offset:3, limit: 3, order: '[["id", "ASC"]]'}, 20);

    cy.log("Get final (part) page, setting limit");
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {}, offset:18, limit: 3, order: '[["id", "ASC"]]'}, expectedRecording.slice(18,20), EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroup2Admin", {where: {}, offset:18, limit: 3, order: '[["id", "ASC"]]'}, 20);
    ...
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {}, offset:0, limit: 3, order: '[["id", "DESC"]]'}, expectedRecording.slice().reverse().slice(0,3), EXCLUDE_IDS);
    cy.log("Reverse sort order, intermediate page");
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {}, offset:3, limit: 3, order: '[["id", "DESC"]]'}, expectedRecording.slice().reverse().slice(3,6), EXCLUDE_IDS);
    cy.log("Reverse sort order, last (part) page");
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {}, offset:18, limit: 3, order: '[["id", "DESC"]]'}, expectedRecording.slice().reverse().slice(18,20), EXCLUDE_IDS);
    ...
    cy.apiRecordingsQueryCheck( "rqGroup2Admin", {where: {}, offset:3, limit: 30, order: '[["id", "DESC"]]'}, expectedRecording.slice().reverse().slice(3,30), EXCLUDE_IDS);
  });

  
  it("Can query by all valid single parameters", () => {
    cy.log("id");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {id:getCreds("rqRecording2").id}}, [expectedRecording2], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {id:getCreds("rqRecording2").id}}, 1);

    cy.log("recordingDateTime");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {recordingDateTime: recording3.recordingDateTime}}, [expectedRecording3], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {recordingDateTime: recording3.recordingDateTime}}, 1);

    cy.log("DeviceId");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {DeviceId: getCreds("rqCamera1").id}, order: '[["id", "ASC"]]'}, [expectedRecording1, expectedRecording2], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {DeviceId: getCreds("rqCamera1").id}}, 2);

    cy.log("GroupId");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {GroupId: getCreds("rqGroup").id}, order: '[["id", "ASC"]]'}, [expectedRecording1  , expectedRecording2, expectedRecording3, expectedRecording4], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {GroupId: getCreds("rqGroup").id}}, 4);

    cy.log("type");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {type: 'audio'}}, [expectedRecording3], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {type: 'audio'}}, 1);

    cy.log("processingState");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {processingState: 'CORRUPT'}}, [expectedRecording2], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {processingState: 'CORRUPT'}}, 1);
    
    cy.log("duration");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {duration: 60}}, [expectedRecording3], EXCLUDE_IDS);
  ...
  it("Can query using operators", () => {
    cy.log("Greater than");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {duration: {"$gt":40}}}, [expectedRecording3], EXCLUDE_IDS);  
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {duration: {"$gt":40}}}, 1);

    cy.log("Less than");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {duration: {"$lt":40}}}, [expectedRecording1], EXCLUDE_IDS);  
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {duration: {"$lt":40}}}, 1);

    cy.log("Less than equal");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {duration: {"$lte":40}}, order: '[["id", "ASC"]]'}, [expectedRecording1, expectedRecording2, expectedRecording4], EXCLUDE_IDS);  
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {duration: {"$lte":40}}}, 3);
  });
  ...
  it.skip("Can query by nested parameters", () => {
    ...
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {"Device.devicename": getTestName("rqCamera1")}, order: '[["id", "ASC"]]'}, [expectedRecording1, expectedRecording2], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin",  {where: {"Device.devicename": getTestName("rqCamera1")}}, 2);

    cy.log("Group.groupname");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {"Group.groupname": getTestName("rqGroup")}, order: '[["id", "ASC"]]'}, [expectedRecording1, expectedRecording2, expectedRecording3], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin",  {where: {"Group.groupname": getTestName("rqGroup")}}, 3);
  ...
  it("Can query by multiple parameters", () => {
    ...
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {DeviceId: getCreds("rqCamera1").id, duration: {"$gte":40}}, order: '[["id", "ASC"]]'}, [expectedRecording2], EXCLUDE_IDS);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {DeviceId: getCreds("rqCamera1").id, duration: {"$gte":40}}, order: '[["id", "ASC"]]'}, 1);
  });

  //TODO: Issue 92: /ap1/v1/recordings/count ignoring tags filter
  it("Can limit query by tags and tagmode", () => {
    cy.log("Tagged as possum");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tags: '["possum"]', order: '[["id", "ASC"]]'}, [expectedRecording2, expectedRecording4], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tags: '["possum"]', order: '[["id", "ASC"]]'}, 1);

    cy.log("Tagged as possum or cat");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tags: '["possum", "cat"]', order: '[["id", "ASC"]]'}, [expectedRecording1, expectedRecording2, expectedRecording4], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tags: '["possum", "cat"]', order: '[["id", "ASC"]]'}, 2);

    cy.log("'Any' tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "any", order:   '[["id", "ASC"]]'}, [expectedRecording1, expectedRecording2, expectedRecording3, expectedRecording4], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tagMode: "any", order:   '[["id", "ASC"]]'}, 4);

    cy.log("'untagged' tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "untagged", order:   '[["id", "ASC"]]'}, [expectedRecording3], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tagMode: "untagged", order:   '[["id", "ASC"]]'}, 1);

    cy.log("'tagged' tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "tagged", order:   '[["id", "ASC"]]'}, [expectedRecording1,   expectedRecording2, expectedRecording4], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tagMode: "tagged", order:   '[["id", "ASC"]]'}, 3);

    cy.log("'no-human' tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "no-human", order:   '[["id", "ASC"]]'}, [expectedRecording1,   expectedRecording2, expectedRecording3], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tagMode: "no-human", order:   '[["id", "ASC"]]'}, 3);

    cy.log("'automatic-only' tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "automatic-only", order:   '[["id", "ASC"]]'}, [expectedRecording1,   expectedRecording2], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tagMode: "automatic-only", order:   '[["id", "ASC"]]'}, 2);

    cy.log("'human-only' tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "human-only", order:   '[["id", "ASC"]]'}, [expectedRecording4], EXCLUDE_IDS);
    //cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tagMode: "human-only", order:   '[["id", "ASC"]]'}, 1);

    cy.log("'automatic+human' tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "automatic+human", order:   '[["id", "ASC"]]'}, [], EXCLUDE_IDS);
    ...
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "automatic-only", tags: '["possum"]', order:   '[["id", "ASC"]]'}, [expectedRecording2], EXCLUDE_IDS);
  ...
  it("Can handle invalid queries", () => {
    ...
    cy.log("Tagmode");
    cy.apiRecordingsQueryCheck( "rqGroupAdmin", {where: {}, tagMode: "rubbish value"}, [], EXCLUDE_IDS, HTTP_Unprocessable);
    cy.apiRecordingsCountCheck( "rqGroupAdmin", {where: {}, tagMode: "rubbish value"}, undefined, HTTP_Unprocessable);
    //cy.log("order");
  ...
  if (Cypress.env("test_using_default_superuser") == true) {
    it("Super-user as user should see only their recordings", () => {
      ...
      cy.apiDeviceUserAdd("rqGroupAdmin", superuser, "rqCamera1b", true, HTTP_OK200,{useRawUserName: true});

      cy.apiRecordingsQueryCheck( superuser, {where: {}, "view-mode":'user'}, [expectedRecording3, expectedRecording4], EXCLUDE_IDS);
      //cy.apiRecordingsCountCheck( superuser, {where: {}, "view-mode":'user'}, 2);
      cy.apiDeviceUserRemove ("rqGroupAdmin", superuser, "rqCamera1b", HTTP_OK200, {useRawUserName: true});
    });
  } else {
