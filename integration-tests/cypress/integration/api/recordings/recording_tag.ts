...
import { ApiRecordingTagRequest, ApiRecordingTagResponse } from "@typedefs/api/tag";
...
describe("Recordings: tag", () => {
  ...
  before(() => {
    ...
    cy.testCreateUserGroupAndDevice("tagGroup2Admin", "tagGroup2", "tagCamera2");
  });
  ...
  it("Cannot add tag for a device you don't have permissions for", () => {
    ...
    cy.apiRecordingTagAdd("tagGroupAdmin","tagRecording5",null,tag1,HTTP_Forbidden);
  ...
  it("Cannot delete tag for a device you don't have permissions for", () => {
    ...
    cy.apiRecordingTagDelete("tagGroupAdmin", "tagRecording6", "tagTag6", HTTP_Forbidden);
  ...
  it("Correct handling of invalid recording id", () => {
    ...
    cy.apiRecordingTagAdd("tagGroupAdmin","999999",null,tag1,HTTP_Forbidden,{useRawRecordingId: true});
  });
  ...
  it("Correct handling of invalid tag data", () => {
      ...
      let ft1=JSON.parse(JSON.stringify(fullTag));
      ...
      let ft2=JSON.parse(JSON.stringify(fullTag));
      ft2["badParam"]="hello";
      cy.apiRecordingTagAdd("tagGroupAdmin","tagRecording10","tagTag10",ft2, HTTP_Unprocessable);

      cy.log("Bad value");
      let ft3=JSON.parse(JSON.stringify(fullTag));
      ft3["startTime"]="hello";
      cy.apiRecordingTagAdd("tagGroupAdmin","tagRecording10","tagTag10",ft3, HTTP_Unprocessable);
