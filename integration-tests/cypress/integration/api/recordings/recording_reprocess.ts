...
describe("Recordings - reprocessing tests", () => {
  before(() => {
    ...
    cy.testCreateUserGroupAndDevice("rrpGroup2Admin", "rrpGroup2", "rrpCamera2");
  ...
  it("Can reprocess a single recording", () => {
    ...
    cy.apiRecordingAdd( "rrpCamera1", recording1, "oneframe.cptv", "rrpRecording1").then(() => {
      ...
      cy.apiReprocess("rrpGroupAdmin", [getCreds("rrpRecording1").id], [getCreds("rrpRecording1").id]);
      ...
      expectedProcessing1 = TestCreateExpectedProcessingData( templateExpectedProcessing, "rrpRecording1", "rrpCamera1", "rrpGroup", null, recording1); 
  ...
  it("Group admin can request reprocess", () => {
    ...
    cy.log("Add recording as device");
    cy.apiRecordingAdd( "rrpCamera1", recording5, "oneframe.cptv", "rrpRecording5").then(() => {
      expectedRecording5 = TestCreateExpectedRecordingData( templateExpectedRecording, "rrpRecording5", "rrpCamera1", "rrpGroup", null, recording5);

      cy.log("Check admin can mark for reprocessing");
      cy.apiReprocess("rrpGroupAdmin", [getCreds("rrpRecording5").id], [getCreds("rrpRecording5").id]);
      ...
      cy.apiRecordingCheck( "rrpGroupAdmin", "rrpRecording5", expectedRecording5, EXCLUDE_IDS);
    });
  });
  ...
  it("Group member can request reprocess", () => {
    ...
    cy.log("Add recording as device");
    cy.apiRecordingAdd( "rrpCamera1", recording6, "oneframe.cptv", "rrpRecording6").then(() => { 
      expectedRecording6 = TestCreateExpectedRecordingData( templateExpectedRecording, "rrpRecording6", "rrpCamera1", "rrpGroup", null, recording6);

      cy.log("Check group member can mark for reprocessing");
      cy.apiReprocess("rrpGroupMember", [getCreds("rrpRecording6").id], [getCreds("rrpRecording6").id]);

      cy.log("Check recording is in reprocess, with existing tracks cleared");
      expectedRecording6.processingState = "reprocess";
      expectedRecording6.processing = false;
      expectedRecording6.Tracks=[];
      cy.apiRecordingCheck( "rrpGroupMember", "rrpRecording6", expectedRecording6, EXCLUDE_IDS);
    });
  });
  ...
  it("Device admin can request reprocess", () => {
    ...
    cy.log("Add recording as device");
    cy.apiRecordingAdd( "rrpCamera1", recording7, "oneframe.cptv", "rrpRecording7").then(() => {
      expectedRecording7 = TestCreateExpectedRecordingData( templateExpectedRecording, "rrpRecording7", "rrpCamera1", "rrpGroup", null, recording7);

      cy.log("Check admin can mark for reprocessing");
      cy.apiReprocess("rrpDeviceAdmin", [getCreds("rrpRecording7").id], [getCreds("rrpRecording7").id]);

      cy.log("Check recording is in reprocess, with existing tracks cleared");
      expectedRecording7.processingState = "reprocess";
      expectedRecording7.processing = false;
      expectedRecording7.Tracks=[];
      cy.apiRecordingCheck( "rrpDeviceAdmin", "rrpRecording7", expectedRecording7, EXCLUDE_IDS);
    });
  });
  ...
  it("Device member can request reprocess", () => {
    ...
    cy.log("Add recording as device");
    cy.apiRecordingAdd( "rrpCamera1", recording8, "oneframe.cptv", "rrpRecording8").then(() => {
      expectedRecording8 = TestCreateExpectedRecordingData( templateExpectedRecording, "rrpRecording8", "rrpCamera1", "rrpGroup", null, recording8);

      cy.log("Check group member can mark for reprocessing");
      cy.apiReprocess("rrpDeviceMember", [getCreds("rrpRecording8").id], [getCreds("rrpRecording8").id]);

      cy.log("Check recording is in reprocess, with existing tracks cleared");
      expectedRecording8.processingState = "reprocess";
      expectedRecording8.processing = false;
      expectedRecording8.Tracks=[];
      cy.apiRecordingCheck( "rrpDeviceMember", "rrpRecording8", expectedRecording8, EXCLUDE_IDS);
    });
  });
  ...
  it("Non members cannot request reprocess", () => {
    ...
    cy.log("Add recording as device");
    cy.apiRecordingAdd( "rrpCamera1", recording9, "oneframe.cptv", "rrpRecording9").then(() => {
      expectedRecording9 = TestCreateExpectedRecordingData( templateExpectedRecording, "rrpRecording9", "rrpCamera1", "rrpGroup", null, recording9);

      cy.log("Check non-member cannot mark for reprocessing");
      cy.apiReprocess("rrpGroup2Admin", [getCreds("rrpRecording9").id], undefined, HTTP_Forbidden);

      cy.log("Check recording is in FINISHED, with existing tracks intact");
      expectedRecording9.processingState = "FINISHED";
      cy.apiRecordingCheck( "rrpDeviceMember", "rrpRecording9", expectedRecording9, EXCLUDE_IDS);
    });
  });
  ...
  it("Failed reprocess requests handled correctly", () => {
    ...
    cy.log("Add recording as device");
    cy.apiRecordingAdd( "rrpCamera1", recording10, "oneframe.cptv", "rrpRecording10").then(() => {
      expectedRecording10 = TestCreateExpectedRecordingData( templateExpectedRecording, "rrpRecording10", "rrpCamera1", "rrpGroup", null, recording10);
    
      cy.log("Check cannot mark non-existant recording for reprocessing");
      cy.apiReprocess("rrpGroupAdmin", [999999], undefined, HTTP_BadRequest, {fail: [999999]});
 
      cy.log("Check mix of valid and invalid recording for reprocessing are accepted / rejected correctly");
      cy.apiReprocess("rrpGroupAdmin", [999999, getCreds("rrpRecording10").id], [getCreds("rrpRecording10").id], HTTP_BadRequest, {fail: [999999]});

      cy.log("Check valid recording is in reprocess, with existing tracks cleared");
      expectedRecording10.processingState = "reprocess";
      expectedRecording10.processing = false;
      expectedRecording10.Tracks=[];
      cy.apiRecordingCheck( "rrpDeviceMember", "rrpRecording10", expectedRecording10, EXCLUDE_IDS);
    });
  });
