...
describe("Devices/query", () => {
  ...
  it("Can match a single device by group+devicename", () => {
    cy.apiCheckDevicesQuery(groupAdmin, [expectedDeviceA1], null, [expectedDeviceA1]);
  });
  ...
  it("Can match multiple devices by group+devicename", () => {
    cy.apiCheckDevicesQuery(groupAdmin, [expectedDeviceA1, expectedDeviceB1], null, [expectedDeviceA1, expectedDeviceB1]);
  });
 
  it("Can match multiple devices in single group", () => {
    cy.apiCheckDevicesQuery(groupAdmin, null, [getTestName(group1)], [expectedDeviceA1, expectedDeviceB1]);
  });
  ...
  it("Group admin can see all and only their group's devices", () => {
    cy.apiCheckDevicesQuery(groupAdmin, null, [getTestName(group1), getTestName(group2)], [expectedDeviceA1, expectedDeviceB1]);
  });

  it("Group user can see all and only their group's devices", () => {
    cy.apiCheckDevicesQuery(groupMember, null, [getTestName(group1), getTestName(group2)], [expectedDeviceA1, expectedDeviceB1]);
  });

  it("Device admin can see all and only their devices", () => {
    cy.apiCheckDevicesQuery(deviceAdmin, null, [getTestName(group1), getTestName(group2)], [expectedDeviceA1]);
  });

  it("Device user can see all and only their devices", () => {
    cy.apiCheckDevicesQuery(deviceMember, null, [getTestName(group1), getTestName(group2)], [expectedDeviceA1]);
  });
  ...
  it("Correctly handles incorrect parameters", () => {
    ...
    cy.apiCheckDevicesQuery(groupMember, "bad value", null, [], 'or', HTTP_Unprocessable);

    //group not  JSON array
    cy.apiCheckDevicesQuery(groupMember, null, "bad value", [], 'or', HTTP_Unprocessable);

    //operator not and or or
    cy.apiCheckDevicesQuery(groupMember, null, [getTestName(group1)], [], 'bad-operator', HTTP_Unprocessable);
  });
});
