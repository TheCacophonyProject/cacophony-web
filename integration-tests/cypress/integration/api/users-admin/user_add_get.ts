...
describe("User: add, get", () => {
  ...
  it("Invalid parameters in user registration", () => {
    cy.apiUserAdd("uagUser6").then(() => {
      ...
      cy.apiUserAdd("uagUser6-1", undefined, undefined, undefined, HTTP_Unprocessable, {additionalParams: {firstName: "bob"}});
      cy.apiUserCheck("uagUser6", getTestName("uagUser6-1"), undefined, [], HTTP_Forbidden, {message: "Could not find a user with a name or id of"});
    });
  });
  ...
  it("Cannot create user with same name (even with different case)", () => {
    cy.apiUserAdd("uagUser7").then(() => {
      ...
      cy.apiUserAdd("uagUser7", undefined, getTestName("firstEmail")+"@email.com", undefined, HTTP_BadRequest, {message: "Username in use"});
      cy.log("Add duplicate user (different case)");
      cy.apiUserAdd("UAGUSER7", undefined, getTestName("secondEmail")+"@email.com", undefined, HTTP_BadRequest, {message: "Username in use"});
    });
  });

  it("Cannot create user with same email (even with different case)", () => {
    cy.apiUserAdd("uagUser8", "password", getTestName("user8")+"@user.com").then(() => {
      cy.log("Add duplicate email");
      cy.apiUserAdd("uagUser8-1", "password", getTestName("user8")+"@user.com", undefined, HTTP_BadRequest, {message:   "Email address in use"});
      cy.log("Add duplicate email (different case)");
      cy.apiUserAdd("uagUser8-2", "password", getTestName("USER8")+"@USER.COM", undefined, HTTP_BadRequest, {message:   "Email address in use"});
    });
  });

  it("Cannot create user with email not matching email format", ()=> {
    cy.log("Blank email");
    cy.apiUserAdd("uagUser8-1", "password", "", undefined, HTTP_Unprocessable, {message:   "body.email: Invalid value"});
    cy.log("leading space");
    cy.apiUserAdd("uagUser8-1", "password", " startwithspace@email.com", undefined, HTTP_Unprocessable, {message:   "body.email: Invalid value"});
    cy.apiUserAdd("uagUser8-1", "password", "noatinemail", undefined, HTTP_Unprocessable, {message:   "body.email: Invalid value"});
    cy.log("Email with no user");
    cy.apiUserAdd("uagUser8-1", "password", "user@", undefined, HTTP_Unprocessable, {message: "body.email: Invalid value"});
    cy.log("Email with no domain");
    cy.apiUserAdd("uagUser8-1", "password", "@email.com", undefined, HTTP_Unprocessable, {message: "body.email: Invalid value"});
  });

  it("Invalid user names rejected", () => {
    cy.log("Cannot add user with no letters");
    cy.apiUserAdd("", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true, message: "'userName' is required"});
    cy.apiUserAdd("1234", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });
    cy.log("Cannot add user with other non-alphanumeric characters");
    cy.apiUserAdd("ABC%", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });
    cy.apiUserAdd("ABC&", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });
    cy.apiUserAdd("ABC>", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });
    cy.apiUserAdd("ABC<", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });
    ...
    cy.apiUserAdd("-ABC", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });
    cy.apiUserAdd("_ABC", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });
    cy.apiUserAdd(" ABC", undefined, undefined, undefined, HTTP_Unprocessable,{ useRawUserName: true });

    cy.log("Can add user with -, _ or space as subsequent letter");
    cy.apiUserAdd("A-B_ C", undefined, getTestName("goodemail")+"@email.com", undefined, HTTP_OK200);
  });

  it("Invaliid passwords rejected", () => {
    cy.log("Blank password");
    cy.apiUserAdd("uagUser9-1", "", undefined, undefined, HTTP_Unprocessable,{ message: "Password must be at least 8 characters long"});
    cy.log("Short password");
    cy.apiUserAdd("uagUser9-1", "1234567", undefined, undefined, HTTP_Unprocessable,{ message: "Password must be at least 8 characters long"});
  });
  ...
  it("Invalid parameters in user get", () => {
    cy.apiUserAdd("uagUser11").then(() => {      
      ...
      cy.apiUserCheck("uagUser11", "goodLookingUserName", undefined, [], HTTP_Forbidden, {useRawUserId: true });
    });
  });
