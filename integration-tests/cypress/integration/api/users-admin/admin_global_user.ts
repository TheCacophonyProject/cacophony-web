...
describe("User: manage global access permissions", () => {
  ...
  if (Cypress.env("running_in_a_dev_environment") == true) {
    it("Super-user can set global read access", () => {
      cy.apiAdminUpdate(superuser, "gapUser1", "read").then(() => {
        const expectedUser=TestCreateExpectedUser("gapUser1", {globalPermission: "read"});
        ...
        cy.apiDeviceInGroupCheck("gapUser1","gapCamera2","gapGroup2", null, expectedDevice2);
        ...
        cy.apiDeviceUserAdd("gapUser1", "gapUser3", "gapCamera2", false, HTTP_Forbidden);

        cy.log("Set back to default (off)");
        cy.apiAdminUpdate(superuser, "gapUser1", "off").then(() => {

        cy.log("Check cannot read globally");
          cy.apiDeviceInGroupCheck("gapUser1","gapCamera2","gapGroup2", null, undefined, {}, HTTP_Forbidden);
        });
      });
    });
  } else {
  ...
  if (Cypress.env("running_in_a_dev_environment") == true) {
    it("Super-user can set global write access", () => {
      cy.apiAdminUpdate(superuser, "gapUser1", "write").then(() => {
        const expectedUser=TestCreateExpectedUser("gapUser1", {globalPermission: "write"});
  ...
  if (Cypress.env("running_in_a_dev_environment") == true) {
    it("Super-user can set permission by userId as well as name", () => {
      cy.apiAdminUpdate(superuser, getCreds("gapUser1").id.toString(), "write", HTTP_OK200,{ useRawUserName: true }).then(() => {
        cy.log("Check can write globally");
        cy.apiDeviceUserAdd("gapUser1", "gapUser3", "gapCamera2", false);
        cy.apiDeviceUserRemove("gapUser1", "gapUser3", "gapCamera2");

        cy.log("Set back to default (off)");
        cy.apiAdminUpdate(superuser, getCreds("gapUser1").id.toString(), "off", HTTP_OK200,{ useRawUserName: true });
      });
    });
  } else {
    it.skip("Super-user can set global write access");
  ...
  if (Cypress.env("running_in_a_dev_environment") == true) {
    it("Correct handling of bad parameters", () => {
      cy.apiAdminUpdate(superuser, "nonExistantUser", "write", HTTP_Forbidden, {message: "Could not find a user with a name"});
      cy.apiAdminUpdate(superuser, "gapUser1", "badPermission", HTTP_Unprocessable, {message: "body.permission: Invalid value"});
    });
  } else {
  ...
  it("Non superuser cannot set global access", () => {
    ...
    cy.apiAdminUpdate("gapUser1", "gapUser1", "read", HTTP_Forbidden, {message: "User is not an admin"});

    cy.log("Check cannot read globally");
    cy.apiDeviceInGroupCheck("gapUser1","gapCamera2","gapGroup2", null, undefined, null, HTTP_Forbidden);
  });
