...
describe("Stations: permissions", () => {
  ...
  it("Permissions: Non-member cannot add, get, update or delete a station", () => {
    ...
    cy.apiGroupStationAdd("saAdmin","saGroup",saStation).then(() => {
      ...
      cy.apiGroupStationsCheck("saNonMember", "saGroup", [], undefined, HTTP_Forbidden);
    ...
    it("Super-user as user should see only their recordings", () => {
      ...
      cy.apiGroupStationAdd("saAdmin","saOnlyGroup",saStation1).then(() => {
        cy.apiGroupStationAdd(superuser,"saSuGroup",saStation2).then(() => {
          ...
          cy.apiGroupStationsCheck(superuser, "saOnlyGroup", [saExpectedStation1]);
     
          cy.log("SU cannot get station in any group by group when in user-mode");
          cy.apiGroupStationsCheck(superuser, "saOnlyGroup", [saExpectedStation1], undefined, HTTP_Forbidden, { additionalParams: {"view-mode": "user" }});
     
          cy.log("SU can get stations in own group by group when in user-mode");
          cy.apiGroupStationsCheck(superuser, "saSuGroup", [saExpectedStation2], undefined, undefined, { additionalParams: {"view-mode": "user" }});
     
          //Station by group and name
          cy.log("Can get station in any group by group, name");
          cy.apiGroupStationCheck(superuser, "saOnlyGroup", "saStation4", saExpectedStation1);
    
          cy.log("Cannot get station in any group by group, name when in user mode");
          cy.apiGroupStationCheck(superuser, "saOnlyGroup", "saStation4", undefined, undefined, HTTP_Forbidden, { additionalParams: {"view-mode": "user" }});
