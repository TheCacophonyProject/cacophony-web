...
let count=0;
let group:string;
let baseGroup:string = "reassign_station_recording_group"
...
describe("Fix location: subsequent recordings", () => {
  ...
  it("Reassign recording: add new recording on same place, after lastRecTime", () => {
    ...
    cy.createDeviceStationRecordingAndFix(Josie, deviceName, manualStationName, secondName, group, oldLocation, newLocation, dayTwo.toISOString(), dayZero.toISOString(), false).then((expectedHistory:DeviceHistoryEntry[]) => {
 
      cy.log("Add new recording in same place, after lastRecordingTime"); 
      cy.log("and check recording uses updated station");
      cy.testUploadRecording(deviceName, {...oldLocation, time: dayThree}, thirdName)
  ...
  it("Reassign recording: add new recording in same place, before lastRecTime", () => {
    ...
    cy.createDeviceStationRecordingAndFix(Josie, deviceName, manualStationName, secondName, group, oldLocation, newLocation, dayTwo.toISOString(), dayZero.toISOString(), false).then((expectedHistory:DeviceHistoryEntry[]) => {
      ...
      cy.log("and check recording uses updated station");
      cy.testUploadRecording(deviceName, {...oldLocation, time: dayOne}, firstName)
      .thenCheckStationNameIs(Josie, getTestName(manualStationName))
      .then(() => {
  ...
  it("Reassign recording: add new recording in same place, before station creation time", () => {
    ...
    //reassign recording from auto station to manual station
    cy.createDeviceStationRecordingAndFix(Josie, deviceName, manualStationName, secondName, group, oldLocation, newLocation, dayTwo.toISOString(), dayOne.toISOString(), false).then((expectedHistory:DeviceHistoryEntry[]) => {
      ...
      cy.testUploadRecording(deviceName, {...oldLocation, time: dayZero}, firstName)
  ...
  it("Reassign recording: add new recording in different place, before lastRecTime", () => {
    ...
    cy.createDeviceStationRecordingAndFix(Josie, deviceName, manualStationName, secondName, group, oldLocation, newLocation, dayTwo.toISOString(), dayZero.toISOString(), false).then((expectedHistory:DeviceHistoryEntry[]) => {
      ...
      .then((newStation:TestNameAndId) => {
         ...
         cy.log("Check devicehistory has a new earlier entry for new location by new recording");
  ...
  it("Reassign recording: add new recording in different place, before station creation time", () => {
    ...
    //reassign recording from auto station to manual station
    cy.createDeviceStationRecordingAndFix(Josie, deviceName, manualStationName, secondName, group, oldLocation, newLocation, dayThree.toISOString(), dayTwo.toISOString(), false).then((expectedHistory:DeviceHistoryEntry[]) => {
  ...
  it("Reassign recording: after subsequent new location & recordings, add past recordings in same location after lastRecTime for that location", () => {
    ...
    cy.createDeviceStationRecordingAndFix(Josie, deviceName, manualStationName, secondName, group, oldLocation, newLocation, dayTwo.toISOString(), dayZero.toISOString(), false).then((expectedHistory:DeviceHistoryEntry[]) => {
      ...
      .then((newStation:TestNameAndId) => {
         ...
         cy.log("Add old recording at old location after last recording at the location (dayThree)");
  ...
  it("Reassign recording: after subsequent new location & recorings, add past recordings in same location before lastRecTime for that location", () => {
    ...
    cy.createDeviceStationRecordingAndFix(Josie, deviceName, manualStationName, secondName, group, oldLocation, newLocation, dayTwo.toISOString(), dayZero.toISOString(), false).then((expectedHistory:DeviceHistoryEntry[]) => {
      ...
      .then((newStation:TestNameAndId) => {
         ...
         cy.log("Add old recording at old location before last recording at the location (dayOne)");
         cy.log("and check recording assigned to re-assigned station");
         cy.testUploadRecording(deviceName, {...oldLocation, time: dayOne}, firstName)
         .thenCheckStationNameIs(Josie, getTestName(manualStationName))
         .then(() => {
