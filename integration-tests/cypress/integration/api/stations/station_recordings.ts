...
describe("Stations: assign recordings to stations", () => {
  ...
  it("Adding a recording in a new location automatically creates a new station, deviceHistory", () => {
    ...
    const recordingTime = new Date(new Date().setDate(new Date().getDate() + 1));
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {
      ...
      cy.testUploadRecording(deviceName, { ...location, time: recordingTime })
        .thenCheckStationIsNew(Josie).then((station:TestNameAndId) => {
          ...
          expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, recordingTime.toISOString(), location, "automatic", station.name);
  ...
  it("Adding a recording within the radius of an existing station assigns the existing station to the recording, updates lastThermalRecording", () => {
    ...
    cy.testUploadRecording(
      ...
      cy.log("Check device has a new valid deviceHistory created");
      expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING, null, "register", null);
      expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, oneDayLater.toISOString(), location, "automatic", station.name);
      cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);

      cy.log("Upload another recording and check assigned to existing station");
      cy.testUploadRecording(deviceName, {...nearbyLocation, time: twoDaysLater}).thenCheckStationNameIs(
        Josie,
        station.name 
      ).then(() => {

        cy.log("Check activeAt unchanged, lastThermalRecording matches new recordingDateTime");
        expectedStation1.lastThermalRecordingTime = twoDaysLater.toISOString(),
        cy.apiStationCheck(Josie, station.name, expectedStation1);

        cy.log("Check new deviceHistoery for new location");
        expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, twoDaysLater.toISOString(), nearbyLocation, "automatic", station.name);
        cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
  ...
  it("Adding an earlier recording within the radius of an existing station assigns the existing station to the recording, does not update lastThermalRecording", () => {
    ...
    const threeDaysTime = new Date(new Date().setDate(new Date().getDate() + 3));
    const expectedStation1 = JSON.parse(JSON.stringify(templateExpectedStation));
    let expectedHistory:DeviceHistoryEntry[]=[];
    expectedStation1.location = location;
    ...
    cy.apiGroupStationAdd(
      ...
      cy.log("Add a matching recording, dated day 3 and check assigned to exsiting station");
      cy.testUploadRecording(
        deviceName,
        { ...location, time: threeDaysTime }
      ).thenCheckStationIdIs(
        Josie,
        stationId
      ).then((station:TestNameAndId) => {
        cy.log("Check activeAt unchanged, lastThermalRecording now matches recordingDateTime");
        expectedStation1.activeAt = oneDaysTime.toISOString(),
        expectedStation1.lastThermalRecordingTime = threeDaysTime.toISOString(),
        cy.apiStationCheck(Josie, station.name, expectedStation1);
 
        cy.log("Check device has a new valid deviceHistory created");
        expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING,   null, "register", null);
        expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, threeDaysTime.toISOString(), location, "automatic", station.name);
        cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);


        cy.log("Upload an earlier recording dated day 2 and check assigned to exsting station");
        cy.testUploadRecording(deviceName, {...nearbyLocation, time: twoDaysTime}).thenCheckStationNameIs(
          Josie,
          station.name
        ).then(() => {
        cy.log("Check station activeAt unchanged, lastThermalRecording not changed (as 2nd recording was earlier than 1st)");
          expectedStation1.activeAt = oneDaysTime.toISOString(),
          expectedStation1.lastThermalRecordingTime = threeDaysTime.toISOString(),
          cy.apiStationCheck(Josie, station.name, expectedStation1);

          cy.log("Check earlier deviceHistory entry added backdated to earlier recording time");
          expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, twoDaysTime.toISOString(), nearbyLocation, "automatic", station.name);
          cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
        });
      });
    });
  ...
  it("Adding an earlier recording matching exactly an existing station assigns the existing station to the recording, does not update lastThermalRecording", () => {
    ...
    cy.apiGroupStationAdd(
      ...
      cy.testUploadRecording(
        ...
        cy.log("Upload an earlier recording dated day 2 and check assigned to exsting station");
        cy.testUploadRecording(deviceName, {...location, time: twoDaysTime}).thenCheckStationNameIs(
          Josie,
          station.name
        ).then(() => {
        cy.log("Check station activeAt unchanged, lastThermalRecording not changed (as 2nd recording was earlier than 1st)");
          ...
          cy.log("Check existing deviceHistory entry backdated to earlier recording time");
          expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, twoDaysTime.toISOString(), location, "automatic", station.name);
          cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
        });
      });
    });
  ...
  it("Adding a recording matching location before automatic start-time extends start-time backwards", () => {
    ...
    cy.testUploadRecording( deviceName, { ...nearbyLocation, time: twoDaysTime })
      ...
      cy.log("Check device has a new valid deviceHistory created");
      expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING,   null, "register", null);
      expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, twoDaysTime.toISOString(), nearbyLocation, "automatic", station.name);
      cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);

      cy.log("Upload another recording and check assigned to same station");
      cy.testUploadRecording(deviceName, { ...nearbyLocation, time: oneDaysTime}).thenCheckStationIdIs(Josie, station.id).then(() => {
        cy.log("Check station start time extended backwards (now day 1)");
        expectedStation1.activeAt=oneDaysTime.toISOString();
        cy.apiStationCheck(Josie, station.name, expectedStation1);

        cy.log("Check existing deviceHistory entry backdated to earlier recording time");
        expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, oneDaysTime.toISOString(), nearbyLocation, "automatic", station.name);
        cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
      });
    });
  });
  ...
  it("Subsequent recording in new location creates new station", () => {
    ...
    let expectedHistory:DeviceHistoryEntry[]=[];
    cy.apiDeviceAdd(deviceName, group);
    cy.apiGroupStationAdd(Josie, group, { name: station1Name, ...location1 },oneDaysTime.toISOString());

    cy.log("Upload recording in location1, check matched to station1");
    cy.testUploadRecording(deviceName, { ...location1Nearby, time: twoDaysTime}).thenCheckStationNameIs(Josie, getTestName(station1Name)).then(() => {

      cy.log("Check device has a new valid deviceHistory created");
      expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING,   null, "register", null);
      expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, twoDaysTime.toISOString(), location1Nearby, "automatic", getTestName(station1Name));
      cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
  
      cy.log("Upload later recording in location2, check new station created");
      cy.testUploadRecording(deviceName, { ...location2, time: threeDaysTime}).thenCheckStationIsNew(Josie).then((station2: TestNameAndId) => {
  
        cy.log("Check device has a new valid deviceHistory created");
        expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, threeDaysTime.toISOString(), location2, "automatic", station2.name);
        cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
      });
    });
  });
  ...
  it("Multiple recordings match multiple stations", () => {
    ...
    let expectedHistory:DeviceHistoryEntry[]=[];
    cy.apiDeviceAdd(deviceName, group);
    cy.apiGroupStationAdd(Josie, group, { name: station1Name, ...location1 },oneDaysTime.toISOString()).then(() => {
      cy.apiGroupStationAdd(Josie, group, { name: station2Name, ...location2 },oneDaysTime.toISOString()).then(() => {
        cy.log("Upload recording in location1, check matched to station1");
        cy.testUploadRecording(deviceName, { ...location1Nearby, time: twoDaysTime}).thenCheckStationNameIs(Josie, getTestName(station1Name)).then(() => {

          cy.log("Upload recording in location2, check matched to station2");
          cy.testUploadRecording(deviceName, { ...location2, time: threeDaysTime}).thenCheckStationNameIs(Josie, getTestName(station2Name)).then(() => {

            cy.log("Check device has two new valid deviceHistory created");
            expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING,   null, "register", null);
            expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, twoDaysTime.toISOString(), location1Nearby, "automatic", getTestName(station1Name));
            expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, threeDaysTime.toISOString(), location2, "automatic", getTestName(station2Name));
            cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
          });
        });
      });
  ...
  it("Adding recording matching location during time-period of now-retired station assigns retired station", () => {
    ...
    cy.apiGroupStationAdd(
      ...
      cy.testUploadRecording(deviceName, {
        ...location,
        time: dayTwo,
      }).thenCheckStationIdIs(Josie, stationId).then(() => {

        cy.log("Check device valid deviceHistory created");
        expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING,   null, "register", null);
        expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, dayTwo.toISOString(), location, "automatic", getTestName(stationName));
        cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
      });
    });
  });
  ...
  it("Verify audio recording correctly assigned to and updates station", () => {
    ...
    cy.apiGroupStationAdd( Josie, group, { name: stationName, ...location }, dayOne.toISOString(),
    ).then(() => {
      ...
      cy.apiRecordingAdd(deviceName, audioRecording, undefined, recording1Name)
        .thenCheckStationNameIs(Josie, getTestName(stationName)).then(() => {
          ...
          cy.log("Check device valid deviceHistory created");
          expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING,  null, "register", null);
          expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, dayTwo.toISOString(), nearbyLocation, "automatic", getTestName(stationName));
          cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
        ...
        .thenCheckStationNameIs(Josie, getTestName(stationName)).then(() => {
          ...
          cy.apiRecordingAdd(deviceName, thermalRecording, undefined, thermalRecordingName)
            .thenCheckStationNameIs(Josie, getTestName(stationName)).then(() => {
              ...
              cy.log("Check station updated correctly with lastThermalRecordingTime");
              expectedStation1.lastAudioRecordingTime = dayTwo.toISOString();   
              expectedStation1.lastThermalRecordingTime = dayThree.toISOString();   
              cy.apiStationCheck(Josie, getTestName(stationName), expectedStation1);

              cy.log("Check device has just one valid deviceHistory location created");
              expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING,   null, "register", null);
              expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, dayTwo.toISOString(), nearbyLocation, "automatic", getTestName(stationName));
              cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
