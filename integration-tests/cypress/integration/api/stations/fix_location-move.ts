  ...
  it("correct location: Move all recordings from auto to manual, after current activeAt", () => {
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {;
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        ...
        expectedHistory.push(TestCreateExpectedHistoryEntry(deviceName, group, firstTime.toISOString(), oldLocation, "automatic", autoStation.name));
        ...
        cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {
          ...
          cy.apiDeviceFixLocation(Josie, deviceName, firstTime.toISOString(), manualStationId.toString(), null, HTTP_OK200, { messages: ["Updated 4 recording(s)"], useRawStationId: true}).  then(() => {
            ...
            expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, firstTime.toISOString(), newLocation, "user", getTestName(manualStationName));
        ...
        .thenCheckStationIsNew(Josie).then((intermediateStation:TestNameAndId) => {
          ...
          cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {
            ...
            expectedManualStation.activeAt = beforeRecordings.toISOString();
              ...
              cy.log("Update first and subsequect recording's location to match manual station");
            cy.apiDeviceFixLocation(Josie, deviceName, firstTime.toISOString(), manualStationId.toString(), null, HTTP_OK200, { messages: ["Updated 2 recording(s)"], useRawStationId: true}).  then(() => {
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {;
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        ...
        cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {
          ...
          cy.log("Update third subsequent recording's location to match manual station");
          cy.apiDeviceFixLocation(Josie, deviceName, thirdTime.toISOString(), manualStationId.toString(), null, HTTP_OK200, { messages: ["Updated 2 recording(s)"], useRawStationId: true}).  then(() => {
            ...
            expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, thirdTime.toISOString(), newLocation, "user", getTestName(manualStationName));
            cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
          ...
          .thenCheckStationIsNew(Josie).then((elsewhereStation:TestNameAndId) => {
            ...
            cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {
              ...
              expectedManualStation.activeAt = beforeRecordings.toISOString();
                ...
                cy.apiDeviceFixLocation(Josie, deviceName, secondTime.toISOString(), manualStationId.toString(), null, HTTP_OK200, { messages: ["Updated 2 recording(s)"], useRawStationId: true}).  then(() => {
                ...
                expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING, null, "register", null);
                expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, firstTime.toISOString(), oldLocation, "automatic", autoStation.name);
                expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, secondTime.toISOString(), newLocation, "user", getTestName(manualStationName));
