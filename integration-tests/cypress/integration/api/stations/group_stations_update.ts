...
describe("Stations: updating", () => {
  ...
  it("Can update a station with new unique name", () => {
    ...
    const station1 = TestCreateStationData("stsuStation", 1);
    const updatedStation1 = TestCreateStationData("stsuUpdatedStation", 1);
    const expectedUpdatedStation1 = TestCreateExpectedStation(
    ...
    cy.apiGroupStationAdd("stsuAdmin", "stsuGroup1", station1).then(
      (stationId: number) => {
        ...
        cy.apiGroupStationsUpdate(
          ...
          undefined,
          undefined,
          { stationIdsAddedOrUpdated: [stationId], stationIdsRetired: [] }
        );
  ...
  it.skip("Can update an exsting station with new unique location", () => {
    ...
    const station1 = TestCreateStationData("stsuStation", 2);
    const expectedStation1 = TestCreateExpectedStation(
      TemplateExpectedStation,
      "stsuStation",
      2
    );
    const updatedStation1 = { name: "stsuStation2", lat: -47, lng: 177 };
    const expectedUpdatedStation1 = TestCreateExpectedStation(
      TemplateExpectedStation,
      "stsuStation",
      2
    ...
    const updatedStation1b = { name: "stsuStation2", lat: -48, lng: 177 };
    const expectedUpdatedStation1b = TestCreateExpectedStation(
      TemplateExpectedStation,
      "stsuStation",
      2
    );
    ...
    cy.log("Adding station");
    cy.apiGroupStationAdd("stsuAdmin", "stsuGroup2", station1).then(
      (initialStationId: number) => {
        cy.log("Updating location");
        cy.apiGroupStationsUpdate("stsuAdmin", "stsuGroup2", [
          updatedStation1,
        ]).then((stationIds: number[]) => {
          ...
          cy.log(
            "Check new station created with the original name and new position"
          );
          cy.apiStationCheck(
            "stsuAdmin",
            stationIds[1].toString(),
            expectedUpdatedStation1,
            undefined,
            undefined,
            { useRawStationId: true }
          ...
          cy.log(
            "Move again and verify do not end up with duplicate named stations"
          );
          cy.apiGroupStationsUpdate("stsuAdmin", "stsuGroup2", [
            updatedStation1b,
          ]).then((stationIds: number[]) => {
            ...
            expectedUpdatedStation1.name =
              expectedUpdatedStation1.name + "_moved";
            cy.apiStationCheck(
              "stsuAdmin",
              stationIds[0].toString(),
              expectedUpdatedStation1,
              undefined,
              undefined,
              { useRawStationId: true }
            ...
            cy.log(
              "Check new station created with latest position and original name"
            );
            cy.apiStationCheck(
              "stsuAdmin",
              stationIds[1].toString(),
              expectedUpdatedStation1b,
              undefined,
              undefined,
              { useRawStationId: true }
            );
  ...
  it("Update with new name and location creates a new station, does not affect old station", () => {
    ...
    const station1 = TestCreateStationData("stsuStation", 3);
    const expectedStation1 = TestCreateExpectedStation(
      TemplateExpectedStation,
      "stsuStation",
      3
    );
    const newStation = TestCreateStationData("stsuStation", 4);
    const expectedNewStation = TestCreateExpectedStation(
      TemplateExpectedStation,
      "stsuStation",
      4
    );
    ...
    cy.log("Adding station");
    cy.apiGroupStationAdd("stsuAdmin", "stsuGroup3", station1).then(
      (initialStationId: number) => {
        cy.log("Updating with new station");
        cy.apiGroupStationsUpdate("stsuAdmin", "stsuGroup3", [newStation]).then(
          (stationIds) => {
  ...
  it("Cannot use update to update a station to have duplicate name in same group (specify only changes)", () => {
    ...
    const station1 = TestCreateStationData("stsuStation", 5);
    const station2 = TestCreateStationData("stsuStation", 6);
    // following has name of station2 but position of station1
    const stationWithSameName = TestCreateStationData("stsuStation", 5);
  ...
  it("Cannot use update to add stations with duplicate name in same group (specify all stations)", () => {
    ...
    const stationWithSameName = TestCreateStationData("stsuStation", 2);
  ...
  it("Can add a station with duplicate name in another group", () => {
    ...
    const station2 = TestCreateStationData("stsuStation", 1);
  ...
  it("Station with duplicate name to retired station", () => {
    ...
    const station1 = TestCreateStationData("stsuStation", 9);
    const stationWithSameName = TestCreateStationData("stsuStation", 10);
    stationWithSameName.name = "stsuStation9";

    const expectedStation1: ApiStationResponse = TestCreateExpectedStation(
      TemplateExpectedStation,
      "stsuStation",
      9
    );
    ...
    cy.apiGroupStationAdd(
      ...
      const station1Id: number = getCreds(getTestName("stsuStation9")).id;
      ...
      cy.log(
        "Can use update to add station2 to have same name as retired station1"
      );
      cy.apiGroupStationsUpdate("stsuAdmin", "stsuGroup9", [
        stationWithSameName,
      ]).then((stationIds: number[]) => {
  ...
  it("No warning on update station with unique location", () => {
    ...
    const station1 = TestCreateStationData("stsuStation", 11);
    const movedStation = TestCreateStationData("stsuStation", 12);
  ...
  it("Warning given for update station too close to another in same group", () => {
    ...
    const station1 = TestCreateStationData("stsuStation", 13);
    const expectedStation1 = TestCreateExpectedStation(
      TemplateExpectedStation,
      "stsuStation",
      13
    );
    const stationTooClose = TestCreateStationData("stsuStation", 13);
  ...
  it("No warning given for station too close in another group", () => {
    ...
    cy.log(
      "Can update station to near posn to station1 but warning no given as groups different"
    );
    cy.apiGroupStationsUpdate(
      "stsuAdmin",
      "stsuGroup14",
  ...
  it("No warning given for station too close to retired station", () => {
    ...
    const stationTooClose = TestCreateStationData("stsuStation", 1);
    ...
    cy.apiGroupStationAdd(
      ...
      cy.log(
        "Can update station to near posn to station1 and no warning given"
      );
      cy.apiGroupStationsUpdate(
        "stsuAdmin",
        "stsuGroup15",
  ...
  it("Automatic station becomes manual when updated", () => {
    ...
    const station2 = TestCreateStationData("stsuStation", 19);
    ...
    cy.testUploadRecording(
      ...
      .thenCheckStationIsNew("stsuAdmin")
      .then(() => {
