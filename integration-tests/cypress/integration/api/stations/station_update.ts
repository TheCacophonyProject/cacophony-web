...
describe("Stations: updating", () => {
  ...
  it("Can update a station with new unique name", () => {
    ...
    cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stuUpdateStation1", expectedStation2);

    cy.log("Check old station name no longer present");
    cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stuStation1", undefined, undefined, HTTP_Forbidden);
  ...
  it("Can update a station with new unique location", () => {
    ...
    cy.log("Check station updated correctly")
    cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stuStation2", expectedStation2);
  ...
  it("Can update a station with both name and new unique location", () => {
    ...
    cy.log("Check station updated correctly")
    cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stuUpdateStation4", expectedStation2);

    cy.log("Check old station name no longer present");
    cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stuStation3", undefined, undefined, HTTP_Forbidden);

  });

  it("Cannot update a station to have duplicate name in same group", () => {
    let station1=TestCreateStationData("stuStation", 5);
    let station2=TestCreateStationData("stuStation", 6);
    let stationWithSameName=TestCreateStationData("stuStation", 6);
    ...
    cy.apiStationUpdate("stuAdmin","stuStation2",stationWithSameName, null, null, null, HTTP_Unprocessable);
  });

  it("Can add a station with duplicate name in another group", () => {
    let station1=TestCreateStationData("stuStation", 7);
    let station2=TestCreateStationData("stuStation", 8);
    let stationWithSameName=TestCreateStationData("stuStation", 8);
    stationWithSameName.name="stuStation7";

    let expectedStation1=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 7);
    let expectedStationWithSameName=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 8);
    ...
    cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stuStation7", expectedStation1);

    cy.log("Check station2 exists")
    cy.apiGroupStationCheck("stuAdmin", "stuGroup2", "stuStation7", expectedStationWithSameName);
  });

  it("Station with duplicate name to retired station", () => {
    let station1=TestCreateStationData("stuStation", 9);
    let station2=TestCreateStationData("stuStation", 10);
    let stationWithSameName=TestCreateStationData("stuStation", 10);
    stationWithSameName.name = "stuStation9";

    let expectedStation1=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 9);
    let expectedStationWithSameName=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 10);
    expectedStationWithSameName.name = getTestName("stuStation9");

    cy.log("Adding station1");
    cy.apiGroupStationAdd("stuAdmin","stuGroup",station1,"2020-01-01T00:00:00.000Z").then(() => {
      let station1Id=getCreds(getTestName("stuStation9")).id;

      cy.log("Adding station2");
      cy.apiGroupStationAdd("stuAdmin","stuGroup",station2)

      cy.log("Retire station1");
      cy.testStationRetire("stuAdmin","stuStation9","2020-02-01T00:00:00.000Z");
      ...
      cy.log("Check station1 exists")
      cy.apiStationCheck("stuAdmin", station1Id.toString(), expectedStation1, null, null, {useRawStationId: true, additionalParams: {"only-active": false}});
      cy.log("Check station2 exists")
  ...
  it("No warning on update station with unique location", () => {
    let station1=TestCreateStationData("stuStation", 11);
    let movedStation=TestCreateStationData("stuStation", 12);

    cy.log("Adding station");
    cy.apiGroupStationAdd("stuAdmin","stuGroup",station1);

    cy.log("Updating station location and check no warnings returned");
    cy.apiStationUpdate("stuAdmin","stuStation11",movedStation,undefined,undefined,undefined,HTTP_OK200,{warnings: "none"});
  });

  it("Warning given for update station too close to another in same group", () => {
    let station1=TestCreateStationData("stuStation", 13);
    let expectedStation1=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 13);
    let station2=TestCreateStationData("stuStation", 14);
    let stationTooClose=TestCreateStationData("stuStation", 13);
    stationTooClose.name="stationTooClose14";

    let expectedStationTooClose=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 13);
    expectedStationTooClose.name=getTestName("stationTooClose14");
    
    cy.log("Adding station1");
    cy.apiGroupStationAdd("stuAdmin","stuGroup",station1).then((station1Id:number) => {

      cy.log("Adding station2");
      cy.apiGroupStationAdd("stuAdmin","stuGroup",station2);
     
      cy.log("Can update station to same posn as station1 but warning given");
      cy.apiStationUpdate("stuAdmin","stuStation14",stationTooClose, undefined, undefined, undefined, HTTP_OK200, {warnings: [`Updated station location is too close to ${getTestName(station1.name)} (#${station1Id}) - recordings may be incorrectly matched`]});
  
      cy.log("Check stations both exist")
      cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stuStation13", expectedStation1);
      cy.apiGroupStationCheck("stuAdmin", "stuGroup", "stationTooClose14", expectedStationTooClose);
    });
  });

  it("No warning given for station too close in another group", () => {
    let station1=TestCreateStationData("stuStation", 15);
    let station2=TestCreateStationData("stuStation", 16);
    let stationTooClose=TestCreateStationData("stuStation", 15);
    ...
    cy.log("Can udpdate station2 to same posn as station1 in different group without warning");
    cy.apiStationUpdate("stuAdmin","stuStation16",stationTooClose, undefined, undefined, undefined, HTTP_OK200, {warnings: "none"});
  });

  it("No warning given for station too close to retired station", () => {
    let station1=TestCreateStationData("stuStation", 17);
    let station2=TestCreateStationData("stuStation", 18);
    let stationWithSameLocation=TestCreateStationData("stuStation", 17);
    stationWithSameLocation.name="stationWithSameLocation18";

    let expectedStation1=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 17);
    let expectedStationWithSameLocation=TestCreateExpectedStation(TemplateExpectedStation,"stationWithSameLocation", 17);
    expectedStationWithSameLocation.name=getTestName("stationWithSameLocation18");
    ...
    cy.apiGroupStationAdd("stuAdmin","stuGroup",station1,"2020-01-01T00:00:00.000Z").then(() => {
      ...
      cy.log("Retire station1");
      cy.testStationRetire("stuAdmin","stuStation17","2020-02-01T00:00:00.000Z");
  ...
  it("Automatic station becomes manual when updated", () => {
    ...
    let expectedStation2=TestCreateExpectedStation(TemplateExpectedStation,"stuStation", 20);
    expectedStation2.lastThermalRecordingTime=recordingTime.toISOString();
    let thisLocation=TestGetLocation(19);

    cy.testUploadRecording("stuCamera1", { ...thisLocation, time: recordingTime }, "saRecording1").thenCheckStationIsNew("stuAdmin").then(() => {
      cy.log("Check autocreated station");
      cy.apiGroupStationCheck("stuAdmin","stuGroup", expectedStation1.name, expectedStation1, undefined, undefined, { useRawStationName: true }).then((stationId:number) => {

        cy.log("Update automatic station");
        cy.apiStationUpdate("stuAdmin",stationId.toString(), station2, undefined, undefined, undefined, undefined, { useRawStationId: true });

        cy.log("Check updated station");
        cy.apiStationCheck("stuAdmin", stationId.toString(), expectedStation2, undefined, undefined, { useRawStationId: true });
      });
    });
  });

  //TODO write this
  it.skip("fromDate and untilDate applied to station as activeAt and retiredAt");
