...
describe("Stations: adding", () => {
  ...
  it("Station with duplicate name to retired station", () => {
    ...
    cy.apiGroupStationAdd("staAdmin","staGroup",station1,"2020-01-01T00:00:00.000Z").then(() => {
      ...
      cy.log("Check station1 exists")
      cy.apiStationCheck("staAdmin", station1Id.toString(), expectedStation1, null, null, {useRawStationId: true, additionalParams: {"only-active": false}});
      cy.log("Check station2 exists")
  ...
  it("Warning given for station too close in same group", () => {
    ...
    cy.log("Adding station");
    cy.apiGroupStationAdd("staAdmin","staGroup",station1).then((station1Id: number) => {
   
      cy.log("Can add duplicate-located station but warning given");
      cy.apiGroupStationAdd("staAdmin","staGroup",stationTooClose, undefined, undefined, HTTP_OK200, {warnings: [`New station is too close to ${getTestName(station1.name)} (#${station1Id}) - recordings may be incorrectly matched`]});

      cy.log("Check stations both exist")
      cy.apiGroupStationCheck("staAdmin", "staGroup", "staStation6", expectedStation1);
      cy.apiGroupStationCheck("staAdmin", "staGroup", "stationTooClose6", expectedStationTooClose);
    });
  });
