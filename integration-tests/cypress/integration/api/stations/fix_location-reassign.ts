...
describe("Device: fix-location (reassign) recordings to correct station", () => {
  ...
  it("correct station: Reassign all recordings from auto to manual, after current activeAt", () => {
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {;
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        cy.testUploadRecording(deviceName, {...oldLocation, time: secondTime}, secondName);
        cy.testUploadRecording(deviceName, {...oldLocation, time: thirdTime}, thirdName);
        cy.testUploadRecording(deviceName, {...oldLocation, time: fourthTime}, fourthName);
        ...
        cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {

          cy.log("Update first and subsequect recording's station");
          cy.apiDeviceFixLocation(Josie, deviceName, firstTime.toISOString(), manualStationId.toString(), oldLocation, HTTP_OK200, { messages: ["Updated 4 recording(s)"], useRawStationId: true}).  then(() => {

            cy.log("Check 4 recording reassigned, moved");
            checkRecordingLocationAndStation(Josie, firstName, oldLocation, getTestName(manualStationName));
            checkRecordingLocationAndStation(Josie, secondName, oldLocation, getTestName(manualStationName));
            checkRecordingLocationAndStation(Josie, thirdName, oldLocation, getTestName(manualStationName));
            checkRecordingLocationAndStation(Josie, fourthName, oldLocation, getTestName(manualStationName));
  ...
  it("correct station: Reassign all recordings from auto to manual station, before current activeAt ", () => {
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {;
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        ...
        cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, afterRecordings.toISOString()).then((manualStationId:number ) => {
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        ...
        cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, afterRecordings.toISOString()).then((manualStationId:number ) => {
          ...
          cy.apiDeviceFixLocation(Josie, deviceName, firstTime.toISOString(), manualStationId.toString(), oldLocation, HTTP_OK200, { messages: ["Updated 4 recording(s)"], useRawStationId: true}).  then(() => {
            ...
            cy.log("check new station activeAt now firstTime");
            expectedManualStation.lastThermalRecordingTime=fifthTime.toISOString();
            expectedManualStation.activeAt=firstTime.toISOString();
            cy.apiStationCheck(Josie, getTestName(manualStationName), expectedManualStation);
  ...
  it("correct station: Reassign earlier recordings from auto to manual", () => {
    ...
    let expectedIntermediateStation:ApiStationResponse=JSON.parse(JSON.stringify(expectedAutoStation));
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {;
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        ...
        .thenCheckStationIsNew(Josie).then((intermediateStation:TestNameAndId) => {
          cy.testUploadRecording(deviceName, {...intermediateLocation, time: fourthTime}, fourthName);
          ...
          cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {
            ...
            expectedManualStation.activeAt = beforeRecordings.toISOString();
              ...
              cy.log("Update first and subsequent recording's station");
            cy.apiDeviceFixLocation(Josie, deviceName, firstTime.toISOString(), manualStationId.toString(), oldLocation, HTTP_OK200, { messages: ["Updated 2 recording(s)"], useRawStationId: true}).  then(() => {
              ...
              cy.log("check new station lastThermalRecordingTime now secondTime");
              expectedManualStation.lastThermalRecordingTime=secondTime.toISOString();
              expectedManualStation.activeAt=beforeRecordings.toISOString();
              cy.apiStationCheck(Josie, getTestName(manualStationName), expectedManualStation);
                
              cy.log("check device history updated to refelect the 2 locations, stations");
              expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING, null, "register", null);
              expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, firstTime.toISOString(), oldLocation, "user", getTestName(manualStationName));
              expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, thirdTime.toISOString(), intermediateLocation, "automatic", intermediateStation.name);
              cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
  
              cy.log("check device location unchanged at intermediate location");
              const expectedDevice=TestCreateExpectedDevice(deviceName, group, true, DeviceType.Thermal);
  ...
  it("correct station: Reasign later recordings from auto to manual", () => {
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {;
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        ...
        cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {
          ...
          cy.apiDeviceFixLocation(Josie, deviceName, thirdTime.toISOString(), manualStationId.toString(), oldLocation, HTTP_OK200, { messages: ["Updated 2 recording(s)"], useRawStationId: true}).  then(() => {

            cy.log("Check 1st 2 recordings unchanged");
            checkRecordingLocationAndStation(Josie, firstName, oldLocation, autoStation.name);
            checkRecordingLocationAndStation(Josie, secondName, oldLocation, autoStation.name);
            cy.log("Check last 2 recordings reassigned");
            checkRecordingLocationAndStation(Josie, thirdName, oldLocation, getTestName(manualStationName));
            checkRecordingLocationAndStation(Josie, fourthName, oldLocation, getTestName(manualStationName));

            cy.log("check old station lastThermalRecordingTime now secondTime");
            expectedAutoStation.lastThermalRecordingTime=secondTime.toISOString();
            ...
            cy.log("check new device history entry added for new location, station");
            expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING, null, "register", null);
            expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, firstTime.toISOString(), oldLocation, "automatic", autoStation.name);
            expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, thirdTime.toISOString(), oldLocation, "user", getTestName(manualStationName));
            cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
  ...
  it("correct station: Reassign intermediate recordings from auto to manual", () => {
    ...
    let expectedIntermediateStation:ApiStationResponse=JSON.parse(JSON.stringify(expectedAutoStation));
    expectedIntermediateStation.location=intermediateLocation;
    let expectedElsewhereStation:ApiStationResponse=JSON.parse(JSON.stringify(expectedAutoStation));
    expectedElsewhereStation.location=elsewhereLocation;
    expectedElsewhereStation.activeAt=fourthTime.toISOString();
    expectedElsewhereStation.lastThermalRecordingTime=fourthTime.toISOString();
    ...
    cy.apiDeviceAdd(deviceName, group).then(() => {;
      ...
      .thenCheckStationIsNew(Josie).then((autoStation:TestNameAndId) => {
        ...
        //2 more recordings at intermediate location
        cy.testUploadRecording(deviceName, {...intermediateLocation, time: secondTime}, secondName)
        .thenCheckStationIsNew(Josie).then((intermediateStation:TestNameAndId) => {
          cy.testUploadRecording(deviceName, {...intermediateLocation, time: thirdTime}, thirdName);
          ...
          .thenCheckStationIsNew(Josie).then((elsewhereStation:TestNameAndId) => {
            ...
            cy.apiGroupStationAdd( Josie, group, { name: manualStationName, ...newLocation }, beforeRecordings.toISOString()).then((manualStationId:number ) => {
              ...
              expectedManualStation.activeAt = beforeRecordings.toISOString();
                ...
                cy.log("Update second and subsequect recording's location to match manual station");
                cy.apiDeviceFixLocation(Josie, deviceName, secondTime.toISOString(), manualStationId.toString(), intermediateLocation, HTTP_OK200, { messages: ["Updated 2 recording(s)"], useRawStationId: true}).  then(() => {
      
                cy.log("Check first recording unchanged");
                checkRecordingLocationAndStation(Josie, firstName, oldLocation, autoStation.name);

                cy.log("Check 2nd and 3rd recordings reassigned");
                checkRecordingLocationAndStation(Josie, secondName, intermediateLocation, getTestName(manualStationName));
                checkRecordingLocationAndStation(Josie, thirdName, intermediateLocation, getTestName(manualStationName));
                cy.log("Check last recording unchanged");
                checkRecordingLocationAndStation(Josie, fourthName, elsewhereLocation, elsewhereStation.name);
                ...
                cy.apiStationCheck(Josie, elsewhereStation.name, expectedElsewhereStation);

                cy.log("check device history updated to new location, station");
                expectedHistory[0]=TestCreateExpectedHistoryEntry(deviceName, group, NOT_NULL_STRING, null, "register", null);
                expectedHistory[1]=TestCreateExpectedHistoryEntry(deviceName, group, firstTime.toISOString(), oldLocation, "automatic", autoStation.name);
                expectedHistory[2]=TestCreateExpectedHistoryEntry(deviceName, group, secondTime.toISOString(), intermediateLocation, "user", getTestName(manualStationName));
                expectedHistory[3]=TestCreateExpectedHistoryEntry(deviceName, group, fourthTime.toISOString(), elsewhereLocation, "automatic", elsewhereStation.name);
                cy.apiDeviceHistoryCheck(Josie, deviceName, expectedHistory);
      
                cy.log("check device location unchanged at elsewhere location");
                const expectedDevice=TestCreateExpectedDevice(deviceName, group, true, DeviceType.Thermal);
  ...
  it("correct-station: Verify empty manual station NOT deleted by fix-loction, reassign", () => {
      ...
      cy.apiGroupStationAdd( Josie, group, { name: oldStationName, ...oldLocation }, beforeRecordings.toISOString()).then((oldStationId:number ) => {
        ...
        cy.apiGroupStationAdd( Josie, group, { name: newStationName, ...newLocation }, beforeRecordings.toISOString()).then((newStationId:number ) => {
          ...
          .thenCheckStationIdIs(Josie, oldStationId).then(() => {
            ...
            cy.apiDeviceFixLocation(Josie, deviceName, firstTime.toISOString(), newStationId.toString(), oldLocation, HTTP_OK200, { messages: ["Updated 1 recording(s)"], useRawStationId: true}).  then(() => {

              cy.log("Check recording reassigned");
              checkRecordingLocationAndStation(Josie, firstName, oldLocation, getTestName(newStationName));
...
function checkRecordingLocationAndStation(userName: string, recordingName: string, expectedLocation:LatLng, stationName: string):any {
