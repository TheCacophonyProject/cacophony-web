...
describe("Activity bookkeeping", () => {
  ...
  it("Ensure that location book-keeping is updated correctly when uploading recordings", async () => {
    ...
    expect(
      device.location,
      "device location was updated correctly",
    ).to.deep.equal(uploadedRecordings[uploadedRecordings.length - 1].location);
    expect(
      device.location,
