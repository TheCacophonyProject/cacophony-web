...
const refreshLogin =
  (api: CacophonyApiClient, _authKey: TestHandle | null = null) =>
  (refreshToken: string) =>
    api.post(
      null,
...
const validateEmailConfirmationToken =
  (api: CacophonyApiClient, authKey: TestHandle | null = null) =>
  (token: string) =>
...
const updateUserFields =
  (api: CacophonyApiClient, authKey: TestHandle | null = DEFAULT_AUTH_ID) =>
  (fields: ApiLoggedInUserUpdates, abortable: boolean = NO_ABORT) => {
    const allowedFields = [
      "displayMode",
      "lastKnownTimezone",
...
const getProjectsForProjectAdminByEmail =
  (api: CacophonyApiClient, authKey: TestHandle | null = DEFAULT_AUTH_ID) =>
  (groupAdminEmail: string, abortable = NO_ABORT) =>
    api.get(
      authKey,
...
const superUserGetProjectsForUserByEmail =
  (api: CacophonyApiClient, authKey: TestHandle | null = DEFAULT_AUTH_ID) =>
  (
    userEmail: string,
    abortable = NO_ABORT,
...
const acceptProjectInvitation =
  (api: CacophonyApiClient, authKey: TestHandle | null = DEFAULT_AUTH_ID) =>
  (groupId: GroupId, abortable = false) =>
    api.post(
      authKey,
...
const confirmAddToProjectRequest =
  (api: CacophonyApiClient, authKey: TestHandle | null = DEFAULT_AUTH_ID) =>
  (membershipRequestJWT: string, abortable = NO_ABORT) =>
    api.post(
      authKey,
...
export default (api: CacophonyApiClient) => {
  ...
  return {
    ...
    withAuth: (authKey: TestHandle) => ({
      ...
      debugGetEmailConfirmationToken: debugGetEmailConfirmationToken(
        api,
        authKey,
      ),
      ...
      getProjectsForProjectAdminByEmail: getProjectsForProjectAdminByEmail(
        api,
        authKey,
      ),
