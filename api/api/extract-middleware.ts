  ...
  if (types && !types.includes(jwtDecoded._type)) {
    return next(
      new AuthenticationError(
        `Invalid JWT access type '${type}', must be ${types.length > 1 ? "one of " : ""
        }${types.map((t) => `'${t}'`).join(", ")}`
      )
    );
  }
      ...
      if (model instanceof ClientError) {
        ...
        if (required) {
          ...
          if (byName && byId) {
            ...
            return next(
              new AuthorizationError(
                `Could not find a ${modelName} with a name or id of '${id}'${id2 ? ` in ${id2}` : ""
                }${forUser ? " for user" : ""}`
              )
            );
          } else if (byId) {
            return next(
              new AuthorizationError(
                `Could not find a ${modelName} with an id of '${id}'${id2 ? ` in ${id2}` : ""
                }${forUser ? " for user" : ""}`
              )
            );
          } else if (byName) {
            return next(
              new AuthorizationError(
                `Could not find a ${modelName} with a name of '${id}'${id2 ? ` in ${id2}` : ""
                }${forUser ? " for user" : ""}`
              )
            );
          } else {
...
const getDevice =
  (forRequestUser: boolean = false, asAdmin: boolean = false, forDevice: boolean = false) =>
    (
