...
import { body, Location, oneOf, Result, ValidationChain } from "express-validator";
...
export const validNameOf = (field: ValidationChain): ValidationChain => (
  nameOf(field)
    ...
    .withMessage((val, {path}) => `'${path}' must only contain letters, numbers, dash, underscore and space.  It must contain at least one letter`)
);
...
export const eitherOf = (...fields: ValidationChain[] | ValidationChain[][]): Middleware & { run: (req: Request) => Promise<Result> } => {
...
export const nameOrIdOf = (field: ValidationChain): Middleware & { run: (req: Request) => Promise<Result> } => (
