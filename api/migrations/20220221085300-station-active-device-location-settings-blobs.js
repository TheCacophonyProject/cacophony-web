...
module.exports = {
  up: async function (queryInterface, Sequelize) {
    ...
    await queryInterface.addColumn("Stations", "automatic", {
      ...
      comment:
        "Set if station is automatically created by recording upload in a new location",
      defaultValue: false,
      allowNull: false,
    });
    await queryInterface.addColumn("Stations", "needsRename", {
      type: Sequelize.BOOLEAN,
      comment:
        "Set if station has an automatically generated name - this is a flag to let the UI know it needs renaming",
      defaultValue: false,
      allowNull: false,
    });
    await queryInterface.addColumn("Stations", "settings", {
      ...
      allowNull: true,
      comment:
        "Group specific settings set for a station for all group members by the group admin",
    });
    await queryInterface.addColumn("Stations", "lastThermalRecordingTime", {
      type: Sequelize.DATE,
      allowNull: true,
      comment:
        "Last time that a thermal recording was received by this station",
    });
    ...
    await queryInterface.addColumn("Groups", "settings", {
      ...
      allowNull: true,
      comment:
        "Group specific settings set for all group members by the group admin",
    });
    ...
    await queryInterface.addColumn("Devices", "uuid", {
      type: Sequelize.INTEGER,
      comment:
        "Immutable ID for this device.  Even if a device is re-registered, this should stay the same",
    });
    ...
    await queryInterface.createTable("DeviceHistory", {
      ...
      fromDateTime: {
        ...
        allowNull: false,
        comment:
          "Earliest time that the device was known to be at this `location`",
      },
      setBy: {
        type: Sequelize.ENUM(
          "automatic",
        ...
        allowNull: false,
        comment:
          "Where the location of this device was set from - a recording upload, manually by the user, or via sidekick",
      },
      deviceName: {
        type: Sequelize.STRING,
        allowNull: false,
        comment:
          "Purely for auditing/debugging purposes, store the device name at the time the location was logged",
      },
      uuid: {
        type: Sequelize.INTEGER,
        allowNull: false,
      ...
      stationId: {
        type: Sequelize.INTEGER,
        comment:
          "Station id of this device history entry.  Note that there are currently no foreign key constraints here, so if a station is deleted this should be updated",
      },
    });
