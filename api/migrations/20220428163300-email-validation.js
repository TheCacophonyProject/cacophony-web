...
module.exports = {
  up: async function (queryInterface, Sequelize) {
    ...
    await queryInterface.addColumn("Users", "lastActiveAt", {
      type: Sequelize.DATE,
      allowNull: true,
      comment: "Updated any time a user logs in, or refreshes their session token"
    });
    ...
    await queryInterface.createTable("UserSessions", {
      refreshToken: {
        ...
        allowNull: false,
        comment: "A unique, one-time-usable token that allows a user to refresh their session JWT token"
      },
      userId: {
