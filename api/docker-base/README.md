...
project on different platforms, and in particular on our build environment github actions (where it build from the docker-base each time).
...
This is by design. By running the command on the base image it saves time as most of the packages are already installed.  
By running the command again when the container is built it makes sure all needed packages are there as some packages may have been added or updated packages since the base image was built.
...
* Build the image
`docker buildx build --platform linux/amd64,linux/arm64/v8 --no-cache . -t cacophonyproject/server-base:<tag> --push`
* Update the project [Dockerfile](../../Dockerfile) to use the new version of `cacophonyproject/server-base`. 
...
Command | Action
---|---
`docker ps -a` | Lists all containers on your system
`docker container prune` | Deletes all stopped containers on your system
`docker images -a` | Lists all images on your system
...
`docker run -it cacophony-web`| Run an interactive session
