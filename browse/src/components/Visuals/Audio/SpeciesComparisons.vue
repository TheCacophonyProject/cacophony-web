<template>
    <div class="species-comparisons-container">
        ...
        <div v-else>
            <species-comparisons-chart :data="chartData" :options="chartOptions" ></species-comparisons-chart>
        </div>
    </div>
</template>
    ...
    methods: {
        async handleParameterChange() {
            ...
            if (this.groupingSelection == "device") {
                this.chartOptions.plugins.title.text = "Percentage of Identified Species per Device (%)"
            } else if (this.groupingSelection == "station") {
                this.chartOptions.plugins.title.text = "Percentage of Identified Species per Station (%)"
            }
            await this.getSpeciesCounts()
            this.loading = false
            ...
            const datasets = [...allWhats].map((what, i) => {
                return {
                    ...
                    data: response.map((r, j) => {
                        ...
                        const proportion = deviceCounts[j] > 0 ? count / deviceCounts[j] : 0
        ...
        updateSelectOptions() {
            if (this.groupingSelection == "device") {
                this.selectOptions = this.devices.map(device => ({
                    "name": `${device.deviceName.slice(0, 25)}${device.deviceName.length > 25 ? '...' : ''}`,
                    "id": device.id
                }))
            } else if (this.groupingSelection == "station") {
                this.selectOptions = this.stations.map(station => ({
                    "name": `${station.name.slice(0, 25)}${station.name.length > 25 ? '...' : ''}`,
                    "id": station.id
                }))
            }
        }
