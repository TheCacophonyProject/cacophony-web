<template>
    <div class="species-time-comparisons-container">
        ...
        <div v-else>
            <species-time-comparisons-chart :data="chartData" :options="chartOptions" ></species-time-comparisons-chart>
        </div>
        <multiselect
        v-model="selected"
    ...
    methods: {
        ...
        async getSpeciesCounts() {
            ...
            const animalList = [...new Set(
                response.map(res => res.result.speciesCountBulk.map(item => item.what))
                ...
                for (const animal of animalList) {
                    for (const res of response) {
                        const index = res.result.speciesCountBulk.findIndex(
                            (item) => item.what === animal && item.from === windowEnds[i].toISOString(),
                        )
                        if (index !== -1) {
                            datasets[animalList.indexOf(animal)].data[i] += res.result.speciesCountBulk[index].count
                        }
                    }
                }
            }
