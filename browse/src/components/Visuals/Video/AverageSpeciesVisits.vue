<template>
    <div class="average-species-visits-container">
        ...
        <div v-else>
            <average-species-visits-chart :data="chartData" :options="chartOptions" ></average-species-visits-chart>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        async getSpeciesData() {
            this.loading = true
            const thermalDevices = this.devices.filter(device => device.type == "audio")
            const iterable = this.groupingSelection == "device" ? thermalDevices : this.stations
            const labels = iterable.map((item) => item.name ? item.name : item.deviceName)
            ...
            var response = await Promise.all(
                requests.map(async req => {
                    ...
                    if (this.groupingSelection == "device") {
                        res = await api.device.getDeviceSpeciesCount(req["id"], req["from"], req["window-size"], false, 'video')
                    } else if (this.groupingSelection == "station") {
                        res = await api.station.getStationSpeciesCount(req["id"], req["from"], req["window-size"], false, 'video')
                    }
            ...
            var daysActiveResponse = await Promise.all(
                daysActiveRequests.map(async req => {
                    var res = null
                    if (this.groupingSelection == "device") {
                        res = await api.device.getDeviceDaysActive(req["id"], req["from"], req["window-size"])
                    } else if (this.groupingSelection == "station") {
            ...
            for (const res of response) {
                for (let i = 0; i < datasets.length; i++) {
                    const speciesCount = res.speciesCounts.find((item) => item.what == datasets[i].label)
                    datasets[i].data.push(speciesCount ? speciesCount.count : 0)
                }
            }

            for (let i = 0; i < datasets.length; i++) {
                datasets[i].data = datasets[i].data.map((item, index) => daysActive[index].daysActive > 0 ? item / daysActive[index].daysActive : 0)
    ...
    computed: {
        windowSize() {
            return Math.ceil((this.toDate.getTime() - this.fromDate.getTime()) / 3600000)
        }
    }
}
